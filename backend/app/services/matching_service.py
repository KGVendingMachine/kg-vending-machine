...
async def _judge_top_candidates(
    *,
    ...
    병렬로 실행한다 — notice_embedding_service.ensure_notices_embedded와
    동일한 원칙(AsyncSession은 동시 접근이 공식적으로 지원되지 않음).
    """
...
async def run_matching(
    ...
    *,
    ...
    # (get_eligible_notices) + 품질 필터를 모두 통과한 `passed`에 한해서만
    # 공고 PDF 임베딩·유사도 검색을 수행한다(전체 후보를 다 임베딩하면
