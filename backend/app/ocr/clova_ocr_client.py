...
# CLOVA 계정 전체 기준 동시 호출 제한(실측: 10개 동시 중 6개가 400 거부,
# 2026-07-11). 호출 지점(여기)에서 전역으로 막아야 여러 호출자가 한도를 안 넘긴다.
...
async def fetch_clova_ocr_text(file_path: str) -> str:
    ...
    10페이지 넘는 PDF는 한 번에 거부되므로 10페이지 단위로 나눠 호출 후 이어붙인다.
    """
...
async def call_clova_ocr_bytes(file_bytes: bytes, image_format: str, name: str) -> str:
    """이미지/PDF 바이트를 CLOVA OCR API에 보내 인식된 텍스트를 반환한다.

    계정 전체 동시 호출 제한을 지키기 위해 전역 세마포어로 감싸 요청한다.
    """
    ...
    async with httpx.AsyncClient(timeout=settings.CLOVA_OCR_TIMEOUT_SECONDS) as client:
        for attempt in range(1, settings.CLOVA_OCR_MAX_RETRIES + 1):
            try:
                ...
                # 여러 페이지 PDF는 페이지마다 images 배열에 항목이 하나씩 생기므로
                # images[0]만 읽으면 나머지 페이지가 유실된다 — 전체를 순회해야 한다.
                page_texts = []
                for image in images:
