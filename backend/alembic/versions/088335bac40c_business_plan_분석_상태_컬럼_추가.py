...
def upgrade() -> None:
    ...
    op.add_column(
        "business_plan", sa.Column("analysis_step", sa.String(length=20), nullable=True)
    )
    op.add_column(
        "business_plan", sa.Column("analysis_error", sa.Text(), nullable=True)
    )
    op.add_column(
        "business_plan", sa.Column("analysis_started_at", sa.DateTime(), nullable=True)
    )
